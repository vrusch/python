...
def get_context(assetId, login_ks, header, phoenixURL):
    ...
    try:
        responseDASH = requests.post(sendDASH, json.dumps(dataDASH), timeout= 5, headers=header)
        RelapsedDASH =  (responseDASH.elapsed.total_seconds())
    except:
        responseDASH = None
        RelapsedDASH = None
    ...
    try:
        responseHLS = requests.post(sendHLS, json.dumps(dataHLS), timeout= 5, headers=header)
        RelapsedHLS =  (responseHLS.elapsed.total_seconds())
    except:
        responseHLS = None
        RelapsedHLS = None
